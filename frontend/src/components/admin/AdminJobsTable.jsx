...
const AdminJobsTable = () => {
    // 1. Get 'allAdminJobs' from the JOB store
    const { allAdminJobs, searchJobByText } = useSelector(store => store.job);
    const [filterJobs, setFilterJobs] = useState(allAdminJobs);
    const navigate = useNavigate();

    useEffect(() => {
        // 2. Filter logic: Ensure allAdminJobs exists before filtering
                ...
                <TableBody>
                    {
                        !filterJobs || filterJobs.length === 0 ? (
                            ...
                            filterJobs.map((job, index) => (
                                // 3. FIX: Use job._id, but fallback to 'index' if id is missing to prevent warning
                                <TableRow key={job?._id || index}>
                                    <TableCell>{job?.company?.name || "NA"}</TableCell>
                                    <TableCell>{job?.title}</TableCell>
                                    <TableCell>{job?.createdAt ? job.createdAt.split("T")[0] : "NA"}</TableCell>
